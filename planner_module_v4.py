...
class Planner:
    def __init__(
        ...
        self.cem = CEM_opt(
            num_action_seq=num_sequence_action,
    ...
    def plan_step(self, state, force_replan=False):
        if not self.planned_actions or force_replan:
            ...
            for seq_idx, act_seq in enumerate(action_sequences):
                ...
                rewards[seq_idx] = (
                    self.propagation_net.propagate(state, act_seq).cpu().detach()
                )
...
if __name__ == "__main__":
    ...
    s = env.reset()
    dyna = BNN(action_space_shape, obs_space_shape, reward_dim=1).to(device)
